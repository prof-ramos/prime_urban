...
- Keep `lib/mock-data.ts` as seed input only during this MVP.
...
TypeScript errors must be fixed rather than silenced. Image optimization stays enabled in production and is disabled only in local development to keep the current Unsplash-heavy workflow simple.
...
const canManageUsers = ({ req }: { req: { user?: { roles?: string[] } | null } }) =>
...
- [ ] **Step 3: Create Payload client helper**
...
- [ ] **Step 5: Create property query functions**
...
- [ ] **Step 6: Create neighborhood query functions**
...
Create `.env.local` from `.env.example` before seeding:
...
Set `PAYLOAD_SECRET`, `DATABASE_URL`, `PAYLOAD_ADMIN_EMAIL`, and `PAYLOAD_ADMIN_PASSWORD` in `.env.local`. Use a strong admin password, change it after the first login, and never commit `.env.local`.
